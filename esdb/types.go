...
const (
	// ConsumerStrategyRoundRobin Distributes events to all client evenly. If the client buffer-size is reached, the
...
// AddReadRoles Adds delete roles.
...
// TruncateBefore The event number from which previous events can be scavenged. This is used to implement soft-deletions
// of streams.
...
// ExcludeSystemEventsFilter filters out event's with system's event types.
...
type PersistentSubscriptionStats struct {
	...
	// Total number of events processed by subscriptions.
	TotalItems int64
